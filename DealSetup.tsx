...
import { generateMailerCopy } from '../services/geminiService';
...
const DealSetup: React.FC = () => {
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    >
                        {OWNER_TYPES.map(type => <option key={type} value={type}>{type}</option>)}
                    </select>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Target City <span className="text-gray-400 font-normal">(Optional)</span></label>
                <input 
                  type="text" 
            ...
            <div className="pt-4 border-t border-gray-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        />
                        <p className="text-xs text-gray-400 mt-1">This will appear on the mailer header.</p>
                    </div>
                    <div>
                        ...
                        />
                         <p className="text-xs text-gray-400 mt-1">Direct line for seller calls.</p>
                    </div>
                </div>
                ...
                <div className="mt-4">
                     ...
                     <input 
                      ...
                      />
                      <p className="text-xs text-gray-400 mt-1">We will generate a QR code for this link if provided.</p>
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {PRICING_PLANS.map((plan) => (
                        <div 
                            ...
                            className={`cursor-pointer p-4 border rounded-lg flex flex-col justify-between transition ${selectedPlan === plan.id ? 'border-primary bg-blue-50 ring-1 ring-primary' : 'border-gray-200 hover:border-gray-300'}`}
                        >
                            <div className="mb-2">
                            ...
                            <div className="text-primary font-bold text-sm">${plan.pricePerUnit.toFixed(2)} / pc</div>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="w-full mt-8 bg-primary text-white py-4 rounded-lg font-bold text-lg hover:bg-blue-800 transition shadow-md disabled:opacity-70 flex justify-center items-center"
            >
              {loading ? (
                <>
  ...
  // STEP 2: Selection & Checkout
  const activeDesign = generatedDesigns.find(d => d.id === selectedDesignId) || generatedDesigns[0];
               ...
               {generatedDesigns.map(design => {
                   ...
                   return (
                       <button
                            ...
                            className={`px-6 py-3 rounded-full text-sm font-bold border-2 transition-all transform hover:-translate-y-1 shadow-sm ${colorClass} ${isSelected ? 'scale-105 shadow-md ring-2 ring-offset-2 ring-transparent' : 'bg-white text-gray-600 border-gray-200'}`}
                       >
                           {design.name}
                       </button>
                   );
        ...
        <div className="lg:w-1/3">
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 sticky top-24">
                ...
                <div className="space-y-6">
                        ...
                        <div className="space-y-3">
                            {PRICING_PLANS.map((plan) => (
                                <div 
                                    key={plan.id}
                                    onClick={() => setSelectedPlan(plan.id)}
                                    className={`cursor-pointer p-3 border rounded-lg flex items-center justify-between transition ${selectedPlan === plan.id ? 'border-primary bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}
                                >
                                    <div>
                                        <div className="font-bold text-sm text-gray-900">{plan.name}</div>
                                        <div className="text-xs text-gray-500">{plan.dimensions}</div>
                                    </div>
                                    <div className="font-bold text-primary text-sm">${plan.pricePerUnit.toFixed(2)}</div>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <button 
                        className="w-full bg-primary text-white py-4 rounded-lg font-bold hover:bg-blue-800 transition shadow-lg transform active:scale-95"
