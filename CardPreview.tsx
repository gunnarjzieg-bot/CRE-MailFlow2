...
import { MailerDesign, BuyingCriteria } from '../types';
...
const CardPreview: React.FC<CardPreviewProps> = ({ design, criteria, onSelect, selected }) => {
  ...
  const bgImage = getImage(criteria.propertyType);
  const targetUrl = criteria.website || 'https://example.com';
  const qrCodeUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(targetUrl)}&bgcolor=ffffff`;
  ...
  const renderLogo = (className: string) => {
    ...
    return <div className={`font-bold uppercase tracking-widest text-gray-500 ${className}`}>{criteria.companyName}</div>;
  };
  ...
  const renderFront = () => {
    ...
    if (design.style === 'modern') {
      return (
        <div className="w-full h-full bg-blue-50 relative flex overflow-hidden font-sans border border-gray-100">
          ...
          {/* Right Content 50% - Filled with Blue-50 to reduce white space */}
      ...
      return (
        <div className="w-full h-full bg-slate-900 text-white relative flex flex-col overflow-hidden font-sans bg-gray-800">
          ...
          <div className="p-6 relative z-10 flex-1 flex flex-col justify-center h-full text-left max-w-[75%]">
            <div className={`inline-block self-start ${accentBg} text-white px-3 py-1 text-[9px] font-bold uppercase tracking-wider mb-3 rounded shadow-sm`}>
                Notice to Owner
            </div>
            <h3 className="text-2xl font-black uppercase leading-none mb-4 text-white drop-shadow-xl">{design.front.headline}</h3>
    ...
    return (
      <div className="w-full h-full bg-stone-100 relative flex flex-col font-serif border border-gray-200">
         ...
         {/* Bottom Content 55% - Warm off-white background */}
         <div className="h-[55%] p-6 flex flex-col bg-stone-100 justify-between items-center text-center">
             <div className="flex-1 flex flex-col justify-center w-full">
                 {criteria.logo ? (
                     ...
                     <div className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mb-3">{criteria.companyName}</div>
                 )}
            ...
            <div className="bg-white border border-stone-200 rounded-lg px-4 py-2 mt-2 shadow-sm w-full max-w-[80%]">
                <span className="block text-[9px] font-bold text-gray-500 uppercase tracking-widest mb-0.5">{design.front.cta}</span>
      ...
      <div className="w-full h-full bg-white flex relative font-sans">
            ...
            <div className="mt-auto flex flex-col gap-2">
                ...
                <div className="flex items-end justify-between gap-2">
                    <div className="flex flex-col gap-1">
                        ...
                        {/* Small Business Info */}
                        {criteria.logo ? (
                            <img src={criteria.logo} className="h-5 w-auto object-contain self-start" />
                        ) : (
                            <div className="text-[8px] font-bold text-gray-900">{criteria.companyName}</div>
                        )}
                    </div>

                    {/* 6 & 7. Secondary CTA & BIGGER QR Code */}
                    <div className="flex flex-col items-center shrink-0">
                        {design.back.secondaryCta && (
                            <span className="text-[7px] font-bold text-blue-900 mb-0.5 text-center max-w-[80px] leading-none">{design.back.secondaryCta}</span>
                        )}
        ...
        <div className="w-2/5 pl-4 p-4 flex flex-col relative h-full bg-white">
            ...
            <div className="absolute top-4 right-4 w-12 h-14 border border-gray-300 bg-white flex flex-col items-center justify-center text-center">
                <span className="text-[6px] uppercase text-gray-400 font-bold leading-tight">Presorted<br/>Standard<br/>US Postage<br/>PAID</span>
            </div>
            ...
            <div className="mt-auto mb-6 pl-2">
                <div className="font-mono text-[10px] text-gray-500 mb-2 leading-relaxed">
                    Current Resident<br/>
                    123 Investment Way<br/>
                    {criteria.targetCity || 'City'}, {criteria.targetState} 90210
                </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div 
        onClick={onSelect}
        // Aspect Ratio 3:2 (standard postcard) -> approximated with aspect-[3/2]
      ...
      >
        ...
        {/* Flip Button Overlay - Bottom Right for better visibility */}
        <button 
            onClick={(e) => {
                e.stopPropagation();
